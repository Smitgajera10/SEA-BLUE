...
function computeRisk(wave: number, windWave: number) {
  ...
  if (wave > 1.2 || windWave > 0.6) return { level: "Moderate", color: "yellow" };
...
export default function DashboardPage() {
  ...
  const [risk, setRisk] = useState<{ level: "Low" | "Moderate" | "High"; color: "green" | "yellow" | "red" }>({
    level: "Low",
    color: "green",
  });
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => setLocation({ lat: pos.coords.latitude, lon: pos.coords.longitude, name: "Your Location" }),
  ...
  const fetchMarine = async (lat: number, lon: number) => {
    ...
    try {
      ...
      // Notify if high risk
      if (typeof window !== "undefined" && Notification.permission === "granted" && r.level === "High") {
        new Notification("⚠️ High Coastal Risk", {
          body: `${location?.name}: Wave ${wave0.toFixed(1)}m | Wind-wave ${windWave0.toFixed(1)}m`,
        });
      }
      ...
      <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-white/90 bg-white/80 border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 grid place-items-center text-white font-bold shadow-md">🌊</div>
            <div>
              <h1 className="text-xl font-bold text-slate-800">Sea-Blue Dashboard</h1>
              <p className="text-xs text-slate-500">Interactive coastal intelligence for India</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => location && fetchMarine(location.lat, location.lon)}
        ...
        {location && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 text-slate-600 mb-2"><MapPin size={18} /><span className="font-medium">Location</span></div>
              <div className="text-slate-800 font-bold text-lg">{location.name}</div>
              <div className="text-xs text-slate-500 mt-1">{location.lat.toFixed(3)}, {location.lon.toFixed(3)}</div>
            </motion.div>

            {/* Wave */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 text-slate-600 mb-2"><Waves size={18} /><span className="font-medium">Wave Height</span></div>
              <div className="text-slate-800 font-bold text-2xl">{data?.hourly?.wave_height?.[0]?.toFixed(1) ?? "-"} <span className="text-sm text-slate-500">m</span></div>
              <div className="text-xs text-slate-500 mt-1">Current reading</div>
            </motion.div>

            {/* Wind */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 text-slate-600 mb-2"><Wind size={18} /><span className="font-medium">Wind Wave</span></div>
              <div className="text-slate-800 font-bold text-2xl">{data?.hourly?.wind_wave_height?.[0]?.toFixed(1) ?? "-"} <span className="text-sm text-slate-500">m</span></div>
              <div className="text-xs text-slate-500 mt-1">Wind-driven waves</div>
            </motion.div>

            {/* Risk */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2 text-slate-600"><Activity size={18} /><span className="font-medium">Risk Level</span></div>
                <div className={`px-2.5 py-1 rounded-full text-xs font-medium border ${riskBadgeClasses}`}>{risk.level}</div>
              </div>
              <div className="text-xs text-slate-500 mt-3">{lastUpdated ? `Updated: ${lastUpdated}` : "—"}</div>
            </motion.div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="rounded-2xl overflow-hidden bg-white shadow-sm border border-slate-100 lg:col-span-2">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="font-semibold text-slate-800">Interactive Coastal Map</h2>
              <div className="text-sm text-slate-500 flex items-center gap-1"><Info size={16}/> Click along the coast to fetch data</div>
            </div>
              ...
              {loading && (
                <div className="absolute inset-0 z-10 grid place-items-center bg-white/80 backdrop-blur-sm">
                  <div className="flex items-center gap-2 text-blue-600 bg-white rounded-full px-4 py-2 shadow-md">
                    ...
                    <span className="text-sm font-medium">Fetching marine data…</span>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-5">
            <div className="rounded-2xl bg-white shadow-sm border border-slate-100 p-5">
              <h3 className="font-semibold text-slate-800 mb-3 flex items-center gap-2"><Shield size={18}/> Risk Legend</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-3 p-2 rounded-lg bg-green-50 border border-green-100">
                  ...
                  <span className="text-slate-700">Low (≤ 1.2m wave & ≤ 0.6m wind-wave)</span>
                </div>
                <div className="flex items-center gap-3 p-2 rounded-lg bg-yellow-50 border border-yellow-100">
                  ...
                  <span className="text-slate-700">Moderate (≤ 2.5m wave or ≤ 1.2m wind-wave)</span>
                </div>
                <div className="flex items-center gap-3 p-2 rounded-lg bg-red-50 border border-red-100">
                  ...
                  <span className="text-slate-700">High (&gt; 2.5m wave or &gt; 1.2m wind-wave)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                ...
                <button onClick={() => setShowSubscribe(false)} className="p-1 rounded-full hover:bg-slate-100 transition">
                  <X size={18} />
                </button>
              </div>
              <form onSubmit={handleSubscribe} className="px-5 py-4 space-y-4">
                <input placeholder="Name" required value={subscribeForm.name} onChange={(e) => setSubscribeForm({ ...subscribeForm, name: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-400" />
                <input placeholder="Email" type="email" required value={subscribeForm.email} onChange={(e) => setSubscribeForm({ ...subscribeForm, email: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-400" />
                <input placeholder="Location (optional)" value={subscribeForm.place} onChange={(e) => setSubscribeForm({ ...subscribeForm, place: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-400" />
                <button disabled={submitting} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-medium transition">
